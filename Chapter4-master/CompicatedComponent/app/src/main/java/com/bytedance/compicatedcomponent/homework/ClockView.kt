...
class ClockView @JvmOverloads constructor(
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        hourPointerLength = panelRadius - 300
        minutePointerLength = panelRadius - 200
        secondPointerLength = panelRadius - 100
        drawDegrees(canvas)

        // todo 1: 每一秒刷新一次，让指针动起来
        val calendar: Calendar = Calendar.getInstance()
        val now: Date = calendar.time
        drawNeedles(canvas, now)
        drawDigit(canvas, now)
    ...
    // 模仿drawDegrees的方法，画一个数字表盘，此处参考了 @卿云帆 的代码
    private fun drawDigit(canvas: Canvas, now:Date) {
        // 画一个数字表盘的背景框
        val paint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
            style = Paint.Style.FILL_AND_STROKE
            strokeCap = Paint.Cap.ROUND
        ...
        val leftx = centerX.toFloat()-110
        val lefty = centerY.toFloat()+160
        val length = 230
        val height = 100
        // 画一个矩形框，并使用Linear_Text_flag来输入文字
        canvas.drawRect(leftx, lefty,leftx+length,lefty + height, paint)
        val paintText = Paint(Paint.LINEAR_TEXT_FLAG).apply {
            color =Color.BLUE
            typeface= Typeface.create(Typeface.SANS_SERIF, Typeface.BOLD)
            textSize = 75f
            alpha = 90
        }
        // 判断分钟是否小于10，若是则加一个0让显示更自然
        if (now.minutes.toInt() < 10){
            val current = now.hours.toString()+":"+"0"+now.minutes.toString()
            canvas.drawText(current,leftx+20,lefty+75, paintText)
        }
        else{
            val current = now.hours.toString()+":"+now.minutes.toString()
            canvas.drawText(current,leftx+20,lefty+75, paintText)
        }
    ...
    private fun drawHoursValues(canvas: Canvas) {
        ...
        val R = (centerX - (resultWidth * 0.05f) - 50).toFloat()
        val number = arrayOf("12", "3", "6", "9")
        for (i in 0..3) {
            val startX = (measuredWidth / 2 + R * sin(Math.PI / 2 * i) -20).toFloat()
            val startY = (measuredHeight / 2 - R * cos(Math.PI / 2 * i) +20).toFloat()
            canvas.drawText(number[i].toString(), startX, startY, paintText)
        }
     ...
     * @param canvas
     */
    private fun drawNeedles(canvas: Canvas, time: Date) {
        val calendar: Calendar = Calendar.getInstance()
        val now: Date = calendar.time
